...
const BASE_ENEMY_SPWAN_TIME: float = 2.0
const ENEMY_SPWAN_TIME_GROWTH: float = -.15
...
func sychronize(to_peer_id: int = -1) -> void:
	if !is_multiplayer_authority():
		return
	...
	if to_peer_id > -1 && to_peer_id != 1:
		_sychronize.rpc_id(to_peer_id, data)
	else:
		_sychronize.rpc(data)
